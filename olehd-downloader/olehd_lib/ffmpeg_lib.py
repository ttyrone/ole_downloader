...
# Since I want to put ffmpeg exe inside github's olehd_lib which is the hierachy nicer than put same path as olehd_gui.py
# then .spec runtime directory need same('.\\olehd_lib\\') as olehd_lib directory name AS run from command `python3 olehd_gui.py` bcoz they need refer same file(you don't want put duplicated ffmpeg exe in 2 places) which sahre the same code "olehd_lib" here.
...
def fixfile(ts_broken_path, ts_reset_path):
